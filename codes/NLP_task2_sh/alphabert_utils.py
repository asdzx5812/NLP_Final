...
def convert_text_area(text_tokens,ans_tokens):
    max_area = len(text_tokens)
    min_area = 0
    ans_tokens_len = len(ans_tokens)
    sos_seq = []
    eos_seq = []
    
    def find_eos(ans_tokens_len,ans_tokens):
        for i in range(ans_tokens_len):
            e_num = text_tokens.count(ans_tokens[-1-i])
            if e_num ==1:
                eos = min(max_area,ans_tokens.index(ans_tokens[-1-i])+i)
                return eos
            elif e_num < 0:
                pass
            else:
                ...
                for j in range(t_num-1):
                    teos = text_tokens.index(ans_tokens[i],teos+1)          
                    eos_seq.append(min(max_area,teos+i))
        eos = pd.Series(eos_seq).mode().loc[0]
        return eos
    ...
    for i in range(ans_tokens_len):
        ...
        if t_num ==1:
            sos = max(min_area,ans_tokens.index(ans_tokens[i])-i)
            eos = find_eos(ans_tokens_len,ans_tokens)
            return sos, eos
        elif t_num<1:
            pass
        else:
            ...
            for j in range(t_num-1):
                ...
                sos_seq.append(max(min_area,tos-i))
...
def post_processing(submit):
    for i in range(len(submit)):
        text = submit.iloc[i]['Text']
        cause = submit.iloc[i]['Cause']
        effect = submit.iloc[i]['Effect']
        
        text_list = text.split()
        cause_list = cause.split()
        effect_list = effect.split()
        
def convert_token2str(tokenlist):
    ans = ''
    for t in tokenlist:
        temp = list(t)
