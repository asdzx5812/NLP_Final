...
class alphabetPooler_pretrain(nn.Module):
    def __init__(self, config):
        super(alphabetPooler_pretrain, self).__init__()
        self.avgpool = nn.Sequential(nn.Linear(2*config['hidden_size'],4*config['hidden_size']),
                                     nn.ReLU(),
                                     nn.Dropout(0.5),
                                     nn.Linear(4*config['hidden_size'],4*config['hidden_size']),
                                     nn.ReLU(),
                                     nn.Dropout(0.5),
                                     nn.Linear(4*config['hidden_size'],100)
                                     )
...
class LSTM_baseModel(nn.Module):
    def __init__(self, config, init_weight= None):
        ...
        self.pretrain = alphabetPooler_pretrain(config)
